...
import {Veil, IVerifier, Poseidon2} from "../src/Veil.sol";
import {IncrementalMerkleTree} from "../src/IncrementalMerkleTree.sol";

contract VeilTest is Test {
    ...
    function setUp() public {
        ...
        veil = new Veil(IVerifier(verifier), poseidon, 20);
    }
    ...
    function testMakeDeposit() public {
        ...
        emit Veil.Deposit(_commitment, 0, block.timestamp);
        veil.deposit{value: veil.DENOMINATION()}(_commitment);
    }
    ...
    function testMakeWithdrawal() public {
        ...
        assertEq(address(veil).balance, veil.DENOMINATION());
        veil.withdraw(payable(address(uint160(uint256(_publicInputs[2])))), _proof, _publicInputs);
        assertEq(recipient.balance, veil.DENOMINATION());
        assertEq(address(veil).balance, 0);
    }
    ...
    function testAnotherAddressSendProof() public {
        ...
        vm.prank(attacker);
        vm.expectRevert();
        veil.withdraw(payable(attacker), _proof, _publicInputs);
    }
}
